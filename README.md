...
0. **The train doesn't move, even with the pantograph raised, for a cause not yet found.** A server-side loop was found forcibly zeroing every part's velocity while the pantograph is down (see above) — real, and fixed — but stopping it didn't restore movement. Every property involved (wheel `HingeConstraint` attachments, radius, torque, actuator type, `Throttle`/`ThrottleFloat`) was checked and came back correct, surfaced as live attributes on the seat so it could be read directly in Studio's Properties panel rather than inferred through another script. Next actual step, not yet done: watch `AssemblyAngularVelocity` on the wheel part itself, directly, while a key is held, to see whether it changes at all.
