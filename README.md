...
6. **Lingotto's scrolling info ticker doesn't animate, and passing the platform's own signal doesn't yet clear its board.** The board data itself populates correctly now (this was chased for a long time as a missing-content problem; it turned out to be that Porta Nuova and Lingotto's signs are built with different internal layouts, one groups fields into a single row, the other splits them across several, and the code only handled the first shape until it was rewritten to scan every layout instead of assuming one). What's still open is narrower: the animated ticker text is set up once at script start and never retried for content that streams in after that point, and the signal guarding Lingotto's own platform wasn't yet wired to clear the board the way Porta Nuova's is. Neither has been confirmed fixed by an actual player driving there, only by code review; the last several attempts to verify board-related fixes by moving a physics part remotely instead of a real character produced misleading results, so this one is left for a real playtest rather than another synthetic one.
